<script lang="ts">
	...
	const { reverse = false }: Props = $props();

	const classes = {
		light: 'me-3 inline h-4 w-4 animate-spin text-white',
		dark: 'me-3 inline h-4 w-4 animate-spin text-gray-200 dark:text-gray-600'
	};

	const getFill = (isPrimary: boolean) => (isPrimary ? 'currentColor' : '#E5E7EB');

	const getOverlayFill = (isPrimary: boolean) => (isPrimary ? '#1C64F2' : 'currentColor');
</script>

{#each [true, false] as isDark}
	<div class={isDark ? 'hidden dark:flex' : 'flex dark:hidden'}>
		<svg
			aria-hidden="true"
			role="status"
			class={reverse ? classes[isDark ? 'light' : 'dark'] : classes[isDark ? 'dark' : 'light']}
			viewBox="0 0 100 101"
			fill="none"
		...
		>
			<path
				d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591
				   ...
				   ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094
				   ...
				   9.08144 50.5908Z"
				{...{ fill: getFill(reverse ? !isDark : isDark) }}
			/>
			<path
				d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116
				   ...
				   93.9676 39.0409Z"
				{...{ fill: getOverlayFill(reverse ? !isDark : isDark) }}
			/>
		</svg>
	</div>
{/each}
